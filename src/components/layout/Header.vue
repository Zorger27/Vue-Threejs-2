...
export default {
  ...
  methods: {
    ...
    clickOutsideHandler(event) {
      if (this.showMenu && !(event.target instanceof HTMLElement && event.target.closest(".burger-menu"))) {
        this.showMenu = false;
      }
    },
    navigateToPortfolio() {
...
<!--        <img :src="getHeaderLogoImage" alt="Header Logo Image">-->
    ...
    <div class="menu" :class="{ 'is-active': showMenu }">
      ...
      <router-link to="/cube3" @click="hideMenu">{{ $t('header.prg02') }}</router-link>
      <router-link to="/cube2" @click="hideMenu">{{ $t('header.prg03') }}</router-link>
      <router-link to="/cube-old" @click="hideMenu">{{ $t('header.prg04') }}</router-link>
